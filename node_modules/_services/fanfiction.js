...
//require works, let's try channeling stuff =D
...
module.exports = {
    ...
    check: function(_url) {
        ...
        go(function * () {
            ...
            if (!adapters[domain]) {
                yield put(checkChan, new Error(t.gettext('Unsupported domain')));
                return checkChan.close();
            }
            ...
            .spread(adapters[domain].process)
                .catch(function(e) {
                    go(function * () {
                        yield put(checkChan, new Error(t.gettext('Failed checking the story')));
                    })
                })
                .then(function(data) {
                    go(function * () {
    ...
    download: function(_infos) {
        var downloadChan = chan();
        go(function * () {
            //concurrently fetch all chapters, because I said so
            Promise.all(_infos.chapterLinks.map(function(link) {
                return request.getAsync(link).then(function() {
                    go(function * () {
                        yield put(downloadChan, {
                            status: 'progress',
                            data: 'progress data'
                        });
                    });
                ...
                total += '<br>';
                return total;
            }, React.renderComponentToString(StoryComponent({
                story: _infos
            })))
